...
{	
    ...
    int offset = my_pe * (NUM_POINTS / n_pes);
...
__global__ void check_solution (float* u, float* l2_norm, int N)
{
	...
	float l2 = 0.0f;
    float u_correct = 0.0f;
    
	if (idx < N) {
        u_correct = std::sin(2.0f * M_PI * (idx + offset) / static_cast<float>(NUM_POINTS - 1));
        l2 = (u[idx] - u_correct) * (u[idx] - u_correct);
    }
